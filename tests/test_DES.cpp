...
TEST(DES, Pic) {
  ...
  ctx.encrypt("pic", "pic.jpg");
  ctx.decrypt("pic_res", "pic");

  ASSERT_TRUE(isFilesEqual("pic.jpg", "pic_res"));
}
...
TEST(DES, Video) {
  ...
  ctx.encrypt("video", "video.webm");
  ctx.decrypt("video_res", "video");

  ASSERT_TRUE(isFilesEqual("video.webm", "video_res"));
}
