...
bool doubleLess(const double a, const double b,
                const double epsilon = default_epsilon) {
  return a < b && !doubleEqual(a, b, epsilon);
}

bool doubleGreaterEq(const double a, const double b,
                     const double epsilon = default_epsilon) {
  return a > b || doubleEqual(a, b, epsilon);
}
...
class IPrimaryTest {
 public:
  // propability [0.5, 1)
  [[nodiscard]] virtual bool isPrimary(
      const boost::multiprecision::cpp_int& number,
...
class AbstractPrimaryTest : public IPrimaryTest {
 ...
 public:
  [[nodiscard]] virtual size_t roundCnt(const double probability) const {
  ...
  [[nodiscard]] bool isPrimary(const boost::multiprecision::cpp_int& number,
    ...
    const boost::multiprecision::cpp_int a = number + 100;
